...
def main():
    neo4j_conn = Neo4jConnection("neo4j://host.docker.internal:7999", "neo4j", "password")
    neo4j_conn.connect()
...
def main():
    ...
    test_query = neo4j_conn.execute_query(Neo4jQueries.get_user_by_email_and_password(),{"email":"admin@admin.com","password":"admin"})
    if not test_query:
        print("No")
    else:
        print(json.dumps(test_query, indent=2))
